...
async def consumer_subscriptions():
    ...
    chat_message_queue__declared = await channel.queue_declare(
        f"{UNIQUE_PREFIX}:internal__messager:chat_message", durable=False)
    ...
    #     f"{UNIQUE_PREFIX}:internal__messager:test_message", durable=False
    # )
    #
    ...
    # )
    #
    # # no_ack=False - поведение по умолчанию, отвечаем принудительно в самом обработчике по мере выполенения
    # # (предпочитаемый вариант)
    # await channel.basic_consume(
    #     simple_message_with_ack_queue__declared.queue, methods.simple_message_with_ack, no_ack=False
    # )
