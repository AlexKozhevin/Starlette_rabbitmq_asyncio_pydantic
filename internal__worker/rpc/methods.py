...
async def pow_chat_message_rpc(message:aiormq.types.DeliveredMessage):
    ...
    hash_result, calculate_elapsed_time = await helpers.PoW(incoming_message, 16).calculate()

    outcoming_message_dict = {}
    outcoming_message_dict["username"] = "internal_worker"
    outcoming_message_dict["message"] = \
        f"POW RPC {incoming_message} hash:{hash_result} elapsed time:{calculate_elapsed_time}"
